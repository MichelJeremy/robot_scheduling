...
void scheduler_run(scheduler_t *ces) {
	...
	scheduler_start(ces);
	while(true) { // loops around
		for (int i = 0; i <= MAJOR_PERIOD; i += 100) {
			printf("%s %d\n", "Value of timer: ", i);
			if (i == 0) { // 0
				...
				scheduler_exec_task(ces, s_TASK_REFINE_ID);q
				...
				if (i == 600) scheduler_exec_task(ces, s_TASK_COMMUNICATE_ID);
				scheduler_exec_task(ces, s_TASK_MISSION_ID);
				scheduler_exec_task(ces, s_TASK_NAVIGATE_ID);
				scheduler_exec_task(ces, s_TASK_AVOID_ID);
				scheduler_exec_task(ces, s_TASK_REPORT_ID);
				...
				scheduler_exec_task(ces, s_TASK_MISSION_ID);
				scheduler_exec_task(ces, s_TASK_NAVIGATE_ID);
				scheduler_exec_task(ces, s_TASK_CONTROL_IASK);
			}
